import React from "react"
import { Box, Button, TextField, Typography,MenuItem, InputLabel, FormControl} from "@mui/material";
import Select from '@mui/material/Select';
import { errorDialog, successDialog } from "../../../../utils";
import { getProducts, saveSupply } from "../../../../api";


const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
    transform: 'translate(-50%, -50%)',
    width: 400,
    bgcolor: '#fff',
    border: '1px solid grey',
    boxShadow: 24,
    p: 4,
  };

export const NewSupply=(props)=>{

    const [nameSupply, setNameSupply] = React.useState("");
    const [quantity, setQuantity] = React.useState("");
    const [unity, setUnity] = React.useState("");
    const [price, setPrice] = React.useState("");

    const save = async ()=>{
        if(nameSupply === ""){
            errorDialog("Falta Nombre")
            return;
        }
        if(unity===""){
            errorDialog(" Seleccione Unidad")
            return;
        }
        if(quantity==="" || isNaN(quantity)){
            errorDialog("Falta  Cantidad")
            return;
        }
        if(price==="" || isNaN(price)){
            errorDialog("Verificar Precio")
            return;
        }
        const data = new FormData();
        data.append("name",nameSupply.substring(0,1).toUpperCase().concat(nameSupply.substring(1).toLowerCase()));
        data.append("unity",unity);
        data.append("quantity",quantity);
        data.append("price",price);

        const resp = await saveSupply(data);
        if(resp===undefined){
            return
        }
        setNameSupply("")
        setQuantity("")
        setUnity("")
        setPrice("")
        props.closeModal();
        successDialog("Insumo Creado!")
        const res =  await getProducts(1);
        props.setProducts(res.data)
        props.setTotalElements(res.pagination.totalElements)
      




   }


    return <div>
        <Box sx={style}>
            <div className='modal-container'>
                <Typography id="modal-modal-title" variant="h5" component="h5">
                    <h4>Nuevo Insumo</h4>
                    
                    <TextField  
                        fullWidth
                        onChange={(e)=>{setNameSupply(e.target.value)}}
                        size="small" 
                        id="name" 
                        label="Nombre" 
                        variant="outlined" />

                    <br/><br/>
                    <FormControl fullWidth>
                        <InputLabel id="demo-simple-select-label">Unidad</InputLabel>
                        <Select
                            labelId="demo-simple-select-label"
                            id="demo-simple-select"
                            value={unity}
                            label="Unidad"
                            size="small"
                            fullWidth
                            onChange={(e)=>{setUnity(e.target.value)}}
                        >
                            <MenuItem value="CC">CC</MenuItem>
                            <MenuItem value="GR">GR</MenuItem>
                            <MenuItem value="MIN">MIN</MenuItem>
                            <MenuItem value="UNIDAD">UNIDAD</MenuItem>
                        </Select>
                    </FormControl>
                    <br/><br/>
                    <TextField  
                        fullWidth
                        onChange={(e)=>{setQuantity(e.target.value)}}
                        size="small" 
                        id="quantity" 
                        label="Cantidad" 
                        variant="outlined" />

                    <br/>
                    <br/>
                    <TextField  
                        fullWidth
                        onChange={(e)=>{setPrice(e.target.value)}}
                        size="small" 
                        id="price" 
                        label="Precio" 
                        variant="outlined" />

                    <br/>
                    <br/>


                    <Button onClick={save} fullWidth variant="contained">Guardar</Button>
                    <br/><br/>
                   
                    
                    
                    
                </Typography>

            </div>
        </Box>       
       
    </div>
}